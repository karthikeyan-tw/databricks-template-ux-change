...
# Authenticate to metadata service IMDSv2 and collect EC2 metadata
IMDSV2_TOKEN=$(curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")
INSTANCE_ID=$(curl -s http://169.254.169.254/latest/meta-data/instance-id -H "X-aws-ec2-metadata-token: $IMDSV2_TOKEN")
AZ=$(curl -s http://169.254.169.254/latest/meta-data/placement/availability-zone -H "X-aws-ec2-metadata-token: $IMDSV2_TOKEN")
REGION=${AZ%?}

# Collect cluster name via EC2 tags
...
# Declare variable for log4j configuration path
LOG4J_PATH=""
...
EOF
  ...
  if [ -e /home/ubuntu/databricks/spark/dbconf/log4j/driver/log4j.properties ] ; then
  ...
  sed -i '/^log4j.appender.publicFile.layout/ s/^/#/g' $LOG4J_PATH
  sed -i '/log4j.appender.publicFile=com.databricks.logging.RedactionRollingFileAppender/a log4j.appender.publicFile.layout=com.databricks.labs.log.appenders.JsonLayout' $LOG4J_PATH
else
  cat > /tmp/amazon-cloudwatch-agent.json << EOF
...
EOF
  ...
  if [ -e /home/ubuntu/databricks/spark/dbconf/log4j/executor/log4j.properties ] ; then
  ...
  sed -i '/^log4j.appender.console.layout/ s/^/#/g' $LOG4J_PATH
  sed -i '/log4j.appender.console.layout=org.apache.log4j.PatternLayout/a log4j.appender.console.layout=com.databricks.labs.log.appenders.JsonLayout' $LOG4J_PATH
fi
